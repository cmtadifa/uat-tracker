...
describe('tester session cookie', () => {
  it('round-trips a signed payload', () => {
    const token = signTesterSession({ projectToken: 'abc-123', testerName: 'Jordan', iat: 1000 })
    const result = verifyTesterSession(token)
    expect(result).toEqual({ projectToken: 'abc-123', testerName: 'Jordan', iat: 1000 })
  })

  it('rejects a tampered payload', () => {
    const token = signTesterSession({ projectToken: 'abc-123', testerName: 'Jordan', iat: 1000 })
    const [payload, signature] = token.split('.')
    const tamperedPayload = Buffer.from(JSON.stringify({ projectToken: 'other-project', testerName: 'Jordan', iat: 1000 })).toString('base64url')
